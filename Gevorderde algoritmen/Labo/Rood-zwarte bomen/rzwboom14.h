...
template <class Sleutel>
RZWboom<Sleutel>::RZWboom(const std::vector<Sleutel>& sleutels, const std::vector<Sleutel>& zwarteSleutels){
	...
	for(int i = 0; i < zwarteSleutels.size(); i++){
...
bool RZWboom<Sleutel>::repOK() const {
	...
	if(*this){
		...
		inorder([&vorige, &ok](const RZWknoop<Sleutel>& knoop) {
			if(*vorige == RZWkleur::rood && knoop.kleur == *vorige){
				throw "Twee rode knopen onder elkaar\n";
			}		
		});
