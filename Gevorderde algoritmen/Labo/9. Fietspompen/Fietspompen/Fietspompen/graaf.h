...
void Graaf<RT>::schrijfKnoop(std::ostream &os, int k) const {
	os << "Burenlijst " << k << "::\n";
	for (Burenlijst::const_iterator it = burenlijsten[k].begin(); it != burenlijsten[k].end(); ++it) {
		os << "  ->" << it->first;
...
void GraafMetKnoopdata<RT, Knoopdata>::schrijfKnoop(std::ostream &os, int k) const {
	...
	for (std::map<int, int>::const_iterator it = this->burenlijsten[k].begin(); it != this->burenlijsten[k].end(); ++it) {
		os << "  ->" << it->first;
