...
public:
	...
	//lege vector terug
	vector<Waarde> geefOptimum(int aantalIteraties = 1);
...
protected:
	std::vector<bool> pompOpKnoop; // MAG HIER EIGENLIJK NIET
};


class Fietspompen : public ProbProb<int> { // bool wordt eig niet gebruikt
public:
	Fietspompen(Graaf<ONGERICHT>& graaf) {
		this->graaf = graaf;
		pompOpKnoop.resize(graaf.aantalKnopen());
	}
private:
	

	Graaf<ONGERICHT> graaf;
	bool isOplossing(const vector<int>& deeloplossing) {
		// overloop elke knoop in graaf
		// als knoop een fietspomp heeft, doe niets
		// als knoop geen fietspomp heeft, bekijk alle zijn buren
		//     1) Heeft één van de buren GEEN fietspomp, dan is het GEEN oplossing
		//     2) Heeft elke buur een fietspomp, dan moet men de volgende knoop onderzoeken


		int i = 0;
		bool nogSteedsGeldigeOplossing = true;
		while (nogSteedsGeldigeOplossing && i < graaf.aantalKnopen()) {
			// bepalen of dat knoop een fietspomp heeft
			if (!pompOpKnoop[i]) {
				std::map<int, int>& buren = graaf[i];

				// al deze buren moeten een pomp bevatten
				std::map<int, int>::const_iterator it = buren.begin();
				while (it != buren.end() && pompOpKnoop[it->first]) {
					it++;
				}
				if (it != buren.end()) {
					nogSteedsGeldigeOplossing = false;
				}
			}
			i++;
		}
		return nogSteedsGeldigeOplossing; // een oplossing als alle waarden goed zijn, dus als de index al die waarden heeft bereikt
	}

	double f(const vector<int>& oplossing) {
		return oplossing.size();
	}
	//gegeven een deeloplossing geeft deze functie een vector met mogelijke voortzettingen
	//samen met hun heuristische waarde
	vector<pair<int, double>> geefVoortzetting(const vector<int>& deeloplossing) {
		std::vector<std::pair<int, double>> voortzetting;
		for(int i = 0; i < graaf.aantalKnopen(); i++){
			if (!pompOpKnoop[i]) {
				std::map<int, int>& buren = graaf[i];
				double heuristiek = buren.size();
				std::map<int, int>::const_iterator it = buren.begin();
				while (it != buren.end()) {
					heuristiek -= pompOpKnoop[it->first];
					it++;
				}
				voortzetting.push_back(std::make_pair(i, heuristiek));
			}		
		}
		return voortzetting;
	}
	//neem een individu en zoek van daaruit lokaal optimum
	void verbeterLokaal(vector<int>& deeloplossing) {

		for (int i = 0; i < deeloplossing.size(); i++) {
			std::map<int, int>& buren = graaf[deeloplossing[i]];
			std::map<int, int>::const_iterator it = buren.begin();
			bool alleBurenBevattenPomp = true;
			while (it != buren.end() && alleBurenBevattenPomp) {
				if (!pompOpKnoop[it->first]) {
					alleBurenBevattenPomp = false;
				}
				it++;
			}

			if (alleBurenBevattenPomp) {
				pompOpKnoop[i] = false;
			}
		}

		deeloplossing.clear();
		for (int i = 0; i < pompOpKnoop.size();i++) {
			if (pompOpKnoop[i] == true) {
				deeloplossing.push_back(i);
			}
...
vector<Waarde> ProbProb<Waarde>::geefProbOplossing() {
	...
	while (!deeloplossingIsOplossing && voortzetting.size() > 0) {
		...
		pompOpKnoop[voortzetting[i].first] = true;
...
template <class Waarde>
vector<Waarde> ProbProb<Waarde>::geefOptimum(int aantalIteraties) {
	vector<Waarde> besteOpl = geefProbOplossing();
	verbeterLokaal(besteOpl);
	for (int i = 1; i < aantalIteraties; i++) {
		vector<Waarde> poging = geefProbOplossing();
		verbeterLokaal(poging);
		if (f(poging) < f(besteOpl)) {
			besteOpl = poging;
		}
	}
