...
% Dit onderdeel bespreekt enkele \textit{classifiers} %die kunnen gebruikt worden bij actieherkenning. Hierbij wordt een leerverzameling $\mathcal{L}$ beschouwd die bestaat uit exemplaren die ofwel in %klasse $+$ of in klasse $-$ behoren, $\mathcal{L} = \mathcal{L}^+ \cup \mathcal{L}^-$. Op deze manier wordt de classificatie beschouwd als een %binaire classificatie. 
%\subsection{Steunvectormachine}
%Oorspronkelijk tracht een steunvectormachine de leerverzameling lineair te scheiden zodat exemplaren uit klasse $+$ zich aan de ene helft van de %scheiding bevinden en de andere exemplaren aan de andere kant. De bijkomende voorwaarde is dat het gekozen hypervlak zo ver mogelijk verwijderd is %van de leerverzameling (figuur \ref{fig:svm}). De  
%
%\begin{figure}[ht]
%	\centering
%	\includegraphics[width=0.5\textwidth]{svm}
%	\caption{Een lineaire steunvectormachine.}
%	\label{fig:svm}
%\end{figure}
